...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const onMove = (event) => {
      ...
      const nx = (event.clientX - ww * 0.5) / (ww * 0.5);
      const ny = (event.clientY - wh * 0.5) / (wh * 0.5);
    ...
    const render = (timeNow) => {
      ...
      if (animationType === 'hover') {
        ...
        if (noiseIsZero) {
          continueRAF = Math.abs(yaw - targetYaw) >= 1e-4 || Math.abs(pitch - targetPitch) >= 1e-4 || Math.abs(roll) >= 1e-4;
        }
      } else if (animationType === '3drotate') {
